...
from helpers import get_psutil_conns, get_lsof_conns, get_friendly_name
...
class ConnectionDetailScreen(Screen):
    ...
    def compose(self) -> ComposeResult:
        ...
        with Vertical():
            yield Static(f"Connection Details: {self.connection_data['friendly']}", id="detail_title")
            ...
            with Container(id="connection_details"):
                ...
                yield Static(f"Friendly Name: {self.connection_data['friendly']}", classes="detail_item")
                yield Static(f"Local Address: {self.connection_data['laddr']}", classes="detail_item")
                yield Static(f"Remote Address: {self.connection_data['raddr']}", classes="detail_item")
            ...
            if self.process_info:
                ...
                with Container(id="process_info"):
                    yield Static(f"Executable: {self.process_info.get('exe', 'N/A')}", classes="detail_item")
                    yield Static(f"Command Line: {self.process_info.get('cmd', 'N/A')}", classes="detail_item")
                    yield Static(f"Status: {self.process_info.get('status', 'N/A')}", classes="detail_item")
                    yield Static(f"User: {self.process_info.get('username', 'N/A')}", classes="detail_item")
                    yield Static(f"Working Directory: {self.process_info.get('cwd', 'N/A')}", classes="detail_item")
                    yield Static(f"Threads: {self.process_info.get('num_threads', 'N/A')}", classes="detail_item")
                    yield Static(f"CPU Usage: {self.process_info.get('cpu_percent', 'N/A')}%", classes="detail_item")
                    yield Static(f"Memory Usage: {round(self.process_info.get('memory_percent', 0), 2)}%", classes="detail_item")
                    
                    # Network connections from this process
                    if self.process_info.get('connections'):
                        yield Static(f"Active Connections: {len(self.process_info.get('connections', []))}", classes="detail_item")
...
class NetTopApp(App):
    ...
    def refresh_connections(self) -> None:
        ...
        status_bar.update(f"Connections: {len(conns)} | Source: {'psutil (root)' if using_root else 'lsof'}")
    ...
    async def on_key(self, event: events.Key) -> None:
        if event.key == "q":
            ...
            if table.cursor_row is not None and table.cursor_row < table.row_count:
                row_key = table.get_row_at(table.cursor_row)[0]  # Get the row key at cursor position
